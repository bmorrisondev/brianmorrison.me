...
func GetPostsByPage(page int) []Post {
	url := fmt.Sprintf("%v/wp-json/wp/v2/posts?page=%v", os.Getenv("WP_URL"), page)
...
func (p *Post) FrontMatter() string {
	str := "---\n"
	str += fmt.Sprintf("slug: %v\n", p.Slug)
	str += fmt.Sprintf("github_url: %v\n", p.Acf.GithubURL)
	str += fmt.Sprintf("video_url: %v\n", p.Acf.VideoURL)
	// str += fmt.Sprintf("series_order: %v", p.Acf.SeriesOrder)
	str += fmt.Sprintf("featured_image: %v\n", p.JetpackFeaturedMediaURL)
	// str += fmt.Sprintf("icon: %v", p.Acf.Icon)
